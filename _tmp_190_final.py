# -*- coding: utf-8 -*-
"""1.9.0 收尾：vsix 0.9.0 + release 1.9.0 + 备份 + 推送 + docs 同步"""
import shutil, glob, os, datetime, subprocess, time, json, io

# ---- 0) tmLanguage 校验 ----
p = r'F:\Vesna\vesna-vscode\syntaxes\vesna.tmLanguage.json'
c = io.open(p, encoding='utf-8').read()
json.loads(c)  # 合法性
assert 'tcp_listen' in c and 'encrypt_dir' in c
print('tmLanguage valid + new builtins present')

# ---- 1) vsix 0.9.0（package.json 版本升）----
pj = r'F:\Vesna\vesna-vscode\package.json'
d = json.load(io.open(pj, encoding='utf-8'))
d['version'] = '0.9.0'
io.open(pj, 'w', encoding='utf-8', newline='\n').write(json.dumps(d, indent=2, ensure_ascii=False))
print('package.json -> 0.9.0')
vsix_dir = r'F:\Vesna\vesna-vscode'
r = subprocess.run(['npx', '--no-install', '@vscode/vsce', 'package', '--out', 'vesna-0.9.0.vsix'],
                   cwd=vsix_dir, capture_output=True, text=True, shell=True)
print('vsce rc=%d' % r.returncode, (r.stdout + r.stderr)[-100:].replace('\n', ' | '))
for old in glob.glob(os.path.join(vsix_dir, 'vesna-0.8.0.vsix')):
    os.remove(old)
print('old vsix removed')

# ---- 2) release 1.9.0 ----
src_rel = r'F:\Vesna\release\vesna-1.8.0-windows-x64'
dst = r'F:\Vesna\release\vesna-1.9.0-windows-x64'
...
shutil.copy(r'F:\Vesna\RELEASE-NOTES-1.9.0.md', os.path.join(dst, 'RELEASE-NOTES-1.9.0.md'))
if os.path.exists(os.path.join(dst, 'RELEASE-NOTES-1.8.0.md')):
    os.remove(os.path.join(dst, 'RELEASE-NOTES-1.8.0.md'))
shutil.copy(os.path.join(vsix_dir, 'vesna-0.9.0.vsix'), os.path.join(dst, 'vesna-0.9.0.vsix'))
if os.path.exists(os.path.join(dst, 'vesna-0.8.0.vsix')):
    os.remove(os.path.join(dst, 'vesna-0.8.0.vsix'))
zipf = r'F:\Vesna\release\vesna-1.9.0-windows-x64.zip'
...
for d2 in ['src', 'bin', 'docs', 'lib', 'examples', 'vesna-vscode', 'tests', 'tools']:
    s = os.path.join(r'F:\Vesna', d2)
    if os.path.exists(s):
        shutil.copytree(s, os.path.join(snap, d2))
for f in ['README.md', 'README.zh-CN.md', 'CHANGELOG.md', 'CHANGELOG.zh-CN.md',
          'CONTRIBUTING.md', 'CONTRIBUTING.zh-CN.md', 'LICENSE', 'VERSION',
