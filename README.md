...
- **Performance**: processes 100k log lines in ~240ms; 183 builtins incl. an HTTP server (`#http_server`); single-file standalone exe; hardened package manager + LSP go-to-definition/signature hints
...
Grab `vesna-1.8.0-windows-x64.zip` from [Releases](../../releases) and extract it anywhere. **Single-file distribution**: the standard library is embedded, `vesna.exe` runs with no side files.
